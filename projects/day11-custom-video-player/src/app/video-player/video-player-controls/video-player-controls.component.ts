  ...
  template: `
    <div class="player__controls">
      ...
      <button
        ...
        title="Toggle Play"
        [textContent]="videoButtonIcon$ | async"
        #toggle
      >
        ►
      </button>
  ...
  implements OnInit, OnDestroy, AfterViewInit
{
    ...
    this.subscription.add(
      fromEvent(this.toggleButton.nativeElement, 'click')
        .pipe(
          ...
          tap((nextAction) =>
            this.videoPlayerService.updateVideoAction(nextAction)
          )
        )
        .subscribe()
    );
    ...
    this.subscription.add(
      fromEvent(progressNativeElement, 'click')
        .pipe(
          ...
          map(({ offsetX }) =>
            this.createProgressBarAction(
              VideoActionEnum.PROGESS_BAR_CLICKED,
              offsetX
            )
          ),
          tap((nextAction) =>
        ...
        .pipe(
          concatMap(() =>
            drag$.pipe(
              ...
              map(({ offsetX }) =>
                this.createProgressBarAction(
                  VideoActionEnum.PROGRESS_BAR_DRAGGED,
                  offsetX
                )
              )
            )
  ...
  ngAfterViewInit(): void {
    ...
    const rangeInputEvents$ = this.rangeInputs.reduce(
      (acc, rangeInput) =>
        acc.concat(
          this.addRangeUpdateEvent(rangeInput, 'change'),
          this.addRangeUpdateEvent(rangeInput, 'mousemove')
        ),
      [] as Observable<VideoAction>[]
    );

    this.subscription.add(
      merge(...skipButtonEvents$, ...rangeInputEvents$)
        .pipe(
          tap((nextAction) =>
            this.videoPlayerService.updateVideoAction(nextAction)
          )
