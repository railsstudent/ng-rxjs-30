  ...
  ngOnInit(): void {
    ...
    this.subscription.add(
      merge(
        ...
        .pipe(
          tap((icon) => this.videoPlayerService.updateVideoButtonIcon(icon))
        )
        .subscribe()
    );
    ...
    this.subscription.add(
      fromEvent(videoNativeElement, 'timeupdate')
        .pipe(
          map(() => {
            const progressTime =
              (videoNativeElement.currentTime / videoNativeElement.duration) *
              100;
            return `${progressTime}%`;
          }),
          tap((flexBasis) =>
            this.videoPlayerService.updateVideoProgressTime(flexBasis)
          )
        )
        .subscribe()
    );
