  ...
  template: `
    <div class="container">
      <ng-container
        *ngTemplateOutlet="
          inventors;
          context: { $implicit: 'Inventors', list: inventorArray$ | async }
        "
      >
      </ng-container>
      <ng-container
        *ngTemplateOutlet="
          inventors;
          context: { $implicit: 'Ordered Inventors', list: ordered$ | async }
        "
      >
      </ng-container>
      <ng-container
        *ngTemplateOutlet="
          inventors;
          context: { $implicit: 'Oldest Inventors', list: oldest$ | async }
        "
      >
      </ng-container>
      ...
      <ng-container
        *ngTemplateOutlet="
          people;
          context: { $implicit: 'People', list: peopleArray$ | async }
        "
      >
      </ng-container>
      <ng-container
        *ngTemplateOutlet="
          people;
          context: {
            $implicit: 'People (Ordered by last name)',
            list: alpha$ | async
          }
        "
      >
...
export class AppComponent {
  ...
  totalYears$ = this.inventors$.pipe(
    sum((acc: number, y) => acc + (y.passed - y.year), 0)
  );
  totalFirstLength$ = this.inventors$.pipe(
    sum((acc: number, y) => acc + y.first.length, 0)
  );
